...
if (!function_exists('render_shortcodes')) {
    function render_shortcodes(?string $html): string
    {
        ...
        // Абзац, в котором стоит ОДИН шорткод, превращаем в div с тем же
        // стилем.
        //
        // Зачем. Форма и каптча выводятся блочной разметкой, а блок не может
        // находиться внутри <p> — браузер закрывает абзац раньше, и вставка
        // оказывается СНАРУЖИ него. Вместе с абзацем терялось выравнивание:
        // кнопка «по центру» в редакторе ставила text-align на абзац, и оно
        // просто не доставалось вставке. Теперь стиль переезжает на обёртку,
        // которая блок держать умеет.
        //
        // Пустым считается не только пробел. Редактор дописывает после плашки
        // неразрывный пробел, чтобы курсор было куда поставить, — и абзац
        // вида «шорткод + &nbsp;» под простое \s* не подходил. Из-за этого
        // выравнивание не срабатывало ровно в том случае, ради которого всё и
        // затевалось: плашку вставили кнопкой и подвинули по центру.
        $blank = '(?:\s|&nbsp;|&#160;|<br\s*/?>)*';
        ...
        $html = preg_replace_callback(
            ...
            function (array $match): string {
                ...
                // Отметка о выравнивании отдельным классом.
                //
                // У формы ширина во всю строку, и text-align на обёртке её не
                // двигает — двигать нечего. Но раз автор нажал «по центру»,
                // он хотел увидеть форму уже и посередине, а не «ничего не
                // произошло». По этому классу оформление сужает её до
                // читаемой ширины и сдвигает. Класс, а не разбор стиля в CSS:
                // в разметке стиль пишется то с пробелом после двоеточия, то
                // без, и ловить его подстрокой — гадание.
                if (preg_match('~text-align\s*:\s*(center|right)~i', $attributes, $align)) {
                    $marker = 'sc-align-' . strtolower($align[1]);

                    // К уже имеющемуся классу метка дописывается ЧЕРЕЗ ПРОБЕЛ:
                    // без него получалось «sc-align-rightpc-lead» — один
                    // несуществующий класс вместо двух рабочих.
                    $attributes = preg_match('~\sclass\s*=\s*(["\'])~i', $attributes)
                        ? preg_replace('~(\sclass\s*=\s*["\'])~i', '$1' . $marker . ' ', $attributes, 1)
                        : $attributes . ' class="' . $marker . '"';
                }

                return '<div' . $attributes . '>' . $match[2] . '</div>';
            },
            $html
        ) ?? $html;
