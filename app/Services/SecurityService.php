...
class SecurityService
{
    ...
    public function detectSqlInjection(string $input): bool
    {
        $patterns = [
            ...
            // Комментарии ловим только там, где они выглядят как обрыв
            // запроса: «-- » в конце строки или /* … */ рядом с ключевым
            // словом. Прежние /\/\*/ и /\*\// срабатывали на ЛЮБОЙ
            // CSS-комментарий: владелец сохранял фрагмент со стилями и
            // получал бан своего IP на час. Слово SCRIPT убрано совсем —
            // оно встречается в обычном тексте, а разметку ловит detectXss.
            '/(--\s*$|--\s+[a-z])/im',
            '/\/\*.{0,40}\b(union|select|insert|update|delete|drop)\b/is',
            // Классический tautology-паттерн (' OR '1'='1, " AND 1=1 и т.п.).
            // Прежние /(\bOR\b.*=.*)/i и /(\bAND\b.*=.*)/i были слишком широкими:
            // ловили любой текст, где где-то дальше встречались "or"/"and" и "=" —
            // например код-сниппет в статье — и банили IP автора на час.
            '/[\'"]\s*(\bOR\b|\bAND\b)\s*[\'"\d]/i',
        ];
