    ...
    private function loadTemplates(Request $request): array
    {
        ...
        foreach ($templateKeys as $key) {
            ...
            $templates[$key] = Cache::remember($cacheKey, 300, function () use ($key, $cart, $request) {
                ...
                // ⚠️ По ДАТЕ, а не по идентификатору. Идентификатор совпадает с
                // датой только пока материалы не правили: стоит владельцу
                // выставить свою дату публикации (а в ленте выпусков это
                // обычное дело), и порядок расходится с тем, что видит
