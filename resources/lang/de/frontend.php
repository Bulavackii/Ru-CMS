<?php

return [

    'header' => [
        'theme_current' => 'aktiv',
        'theme_fallback' => 'Standard',
        'menu' => 'Menü',
        'tagline' => 'Inhalt & Verwaltung',
        'search' => 'Suche…',
        'search_action' => 'Suchen',
        'account' => 'Konto',
        'account_title' => 'Persönliches Konto',
        'admin' => 'Adminbereich',
        'admin_title' => 'Verwaltungsbereich',
        'login' => 'Anmelden',
        'register' => 'Registrieren',
        'logout' => 'Abmelden',
        'cart' => 'Warenkorb',
        'language' => 'Sprache',
        'theme' => 'Design',
        'theme_default' => 'Wie auf der Website',
        'theme_site' => 'Standard',
        'home_aria' => 'Zur Startseite',
        'logo_alt' => 'Logo',
    ],

    'footer' => [
        'developer' => 'Entwickler',
        'rights' => 'alle Rechte vorbehalten',
        'contacts' => 'Kontakt',
        'write_us' => 'Schreiben Sie uns',
        'email' => 'Ihre E-Mail',
        'send' => 'Senden',
        'sending' => 'Wird geöffnet…',
        'to_top' => 'Nach oben',
        'bad_email' => 'Geben Sie eine gültige E-Mail-Adresse ein',
        'mail_subject' => 'Nachricht von der Website',
        'mail_from' => 'Meine E-Mail:',
        'mail_body' => 'Nachricht:',
    ],

    'news' => [
        'all' => 'Alle Neuigkeiten',
        'copy_link' => 'Link kopieren',
        'details' => 'Details',
        'empty' => 'Noch keine veröffentlichten Beiträge.',
        'in_stock' => 'Verfügbar:',
        'latest' => 'Neueste Beiträge',
        'link_copied' => 'Link kopiert',
        'next' => 'Weiter',
        'no_category' => 'Ohne Kategorie',
        'no_image' => 'Kein Bild',
        'prev' => 'Zurück',
        'preview' => 'Beitragsvorschau',
        'published_at' => 'Veröffentlicht',
        'published_on' => 'Veröffentlichungsdatum',
        'quantity' => 'Menge:',
        'read_more' => 'Weiterlesen',
        'reading_time' => 'Min. Lesezeit',
        'section' => 'Neuigkeiten',
        'share' => 'Teilen:',
    ],

    'search' => [
        'title' => 'Suche',
        'title_with' => 'Suche: :query',
        'subtitle' => 'Wir durchsuchen Beiträge und Seiten',
        'found' => 'Treffer gefunden:',
        'placeholder' => 'Wonach suchen Sie?',
        'submit' => 'Finden',
        'start' => 'Suche starten',
        'start_hint' => 'Geben Sie ein Wort oder eine Phrase ein — wir zeigen passende Beiträge und Seiten.',
        'too_short' => 'Suchanfrage zu kurz',
        'too_short_hint' => 'Geben Sie mindestens zwei Zeichen ein.',
        'nothing' => 'Nichts gefunden',
        'nothing_hint' => 'Keine Treffer für ":query".',
        'tip_spelling' => 'Schreibweise prüfen',
        'tip_one_word' => 'Versuchen Sie ein einzelnes Wort statt einer Phrase',
        'tip_general' => 'Verwenden Sie einen allgemeineren Begriff',
        'pages_section' => 'Seiten der Website',
    ],

    'account' => [
        'edit_hint' => 'Kontakte und Firmendaten — nur für den Administrator sichtbar',
        'g_contacts' => 'Kontakte',
        'g_company' => 'Firmendaten',
        'f_address' => 'Adresse',
        'f_phone' => 'Telefon',
        'f_zip' => 'Postleitzahl',
        'f_ceo' => 'Geschäftsführer',
        'f_legal_addr' => 'Rechtliche Adresse',
        'f_actual_addr' => 'Tatsächliche Adresse',
        'f_okato' => 'OKATO',
        'save' => 'Speichern',
        'hello' => 'Hallo, :name',
        'spent' => 'Gesamtsumme',
        'last_order' => 'Letzte Bestellung',
        'history_hint' => 'Wann, von wo und mit welchem Gerät auf das Konto zugegriffen wurde',
        'history_empty_hint' => 'Einträge erscheinen nach der ersten Anmeldung.',
        'history_total' => 'Einträge insgesamt',
        'h_success' => 'Erfolgreich',
        'h_failed' => 'Fehlgeschlagen',
        'h_blocked' => 'Blockiert',
        'h_suspicious' => 'Verdächtig',
        'h_unknown' => 'Unbekannt',
        'pass_hint' => 'Wählen Sie ein neues Passwort für die Anmeldung',
        'current_pass' => 'Aktuelles Passwort',
        'new_pass' => 'Neues Passwort',
        'confirm_pass' => 'Passwort bestätigen',
        'individual' => 'Privatperson',
        'legal_entity_type' => 'Juristische Person',
        'company' => 'Firma',
        'amount' => 'Summe',
        'change_password' => 'Passwort ändern',
        'date' => 'Datum',
        'delivery' => 'Lieferung',
        'edit_profile' => 'Profil bearbeiten',
        'email' => 'E-Mail',
        'history_empty' => 'Der Anmeldeverlauf ist leer',
        'inn' => 'Steuernummer',
        'legal_entity' => 'Ich bin eine juristische Person',
        'login_history' => 'Anmeldeverlauf',
        'my_orders' => 'Meine Bestellungen',
        'name' => 'Name',
        'ogrn' => 'Registernummer',
        'order_number' => 'Nr.',
        'orders_all' => 'Alle Bestellungen',
        'orders_empty' => 'Sie haben noch keine Bestellungen.',
        'orders_last' => 'Letzte Bestellungen',
        'org_name' => 'Firmenname',
        'organization' => 'Firmendaten',
        'payment' => 'Zahlung',
        'quantity' => 'Menge',
        'status' => 'Status',
        'title' => 'Persönliches Konto',
        'user_type' => 'Kontotyp',
        'subtitle' => 'Ihre Daten und Bestellhistorie',
        'profile' => 'Profil',
        'actions' => 'Aktionen',
        'edit' => 'Daten bearbeiten',
        'edit_org' => 'Firmendaten',
        'change_pass' => 'Passwort ändern',
        'orders_count' => 'Bestellungen',
        'orders_none_hint' => 'Ihre Bestellungen erscheinen hier nach dem ersten Kauf.',
        'to_catalog' => 'Zu den Nachrichten',
        'st_pending' => 'Wartet auf Zahlung',
        'st_paid' => 'Bezahlt',
        'st_completed' => 'Abgeschlossen',
        'st_cancelled' => 'Storniert',
        'st_unknown' => 'In Bearbeitung',
        'order_sum' => 'Bestellsumme',
        'items' => 'Artikel',
    ],

    'common' => [
        'cms_tagline' => 'Modulares CMS',
        'fit_screen' => 'An Bildschirm anpassen',
        'download' => 'Herunterladen',
        'home' => 'Startseite',
        'more' => 'Details',
        'back' => 'Zurück',
        'close' => 'Schließen',
        'open' => 'Öffnen',
        'send' => 'Senden',
        'loading' => 'Wird geladen…',
    ],

    'cart' => [
        'free_delivery_from' => 'Kostenlose Lieferung ab :sum ₽',
        'weight_exceeded' => 'Das Gewichtslimit ist überschritten. Maximal: :limit kg',
        'region_unavailable' => 'Lieferung in diese Region ist nicht möglich',
        'no_delivery' => 'Es wurden noch keine Versandarten eingerichtet — bitte kontaktieren Sie uns.',
        'order_not_found' => 'Die Bestellung wurde nicht gefunden.',
        'payment_confirmed' => 'Die Zahlung wurde bestätigt. Vielen Dank!',
        'payment_cancelled' => 'Die Zahlung wurde abgebrochen.',
        'payment_pending' => 'Wir warten auf die Bestätigung der Zahlung. Sobald die Bank antwortet, wird der Bestellstatus aktualisiert.',
        'payment_start_failed' => 'Die Zahlung konnte nicht gestartet werden. Die Bestellung wurde angelegt — wir melden uns bei Ihnen.',
        'no_payment' => 'Es wurden noch keine Zahlungsarten eingerichtet — bitte kontaktieren Sie uns.',
        'choose_delivery' => 'Versandart wählen…',
        'choose_payment' => 'Zahlungsart wählen…',
        'delivery_method' => 'Versandart',
        'empty' => 'Ihr Warenkorb ist leer.',
        'fee' => 'Gebühr:',
        'payment_method' => 'Zahlungsart',
        'quantity' => 'Menge:',
        'sum' => 'Zwischensumme:',
        'title' => 'Ihr Warenkorb',
        'total' => 'Gesamtbetrag:',
    ],

    'a11y' => [
        'background' => 'Hintergrund ändern',
        'colorblind' => 'Modus für Farbenblinde',
        'dyslexia' => 'Schrift für Legasthenie',
        'read_mode' => 'Lesemodus',
        'spacing' => 'Größere Abstände',
        'reset' => 'Alles zurücksetzen',
        'widget' => 'Barrierefreiheit',
        'title' => 'Einstellungen zur Barrierefreiheit',
        'text_size' => 'Textgröße',
        'speak_selection' => 'Markierten Text vorlesen',
        'speak_page' => 'Ganze Seite vorlesen',
        'contrast' => 'Hoher Kontrast',
        'monochrome' => 'Monochrom',
        'sepia' => 'Sepia',
        'reading_mask' => 'Lesemaske',
        'link_highlight' => 'Links hervorheben',
        'speak' => 'Vorlesen',
        'stop' => 'Stopp',
        'on' => 'Aktivieren',
        'off' => 'Deaktivieren',
        'show' => 'Anzeigen',
        'hide' => 'Ausblenden',
    ],
];
