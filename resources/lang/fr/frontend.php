<?php

return [

    'header' => [
        'theme_current' => 'active',
        'theme_fallback' => 'par défaut',
        'menu' => 'Menu',
        'tagline' => 'Contenu & Gestion',
        'search' => 'Recherche…',
        'search_action' => 'Rechercher',
        'account' => 'Compte',
        'account_title' => 'Espace personnel',
        'admin' => 'Administration',
        'admin_title' => 'Panneau d\'administration',
        'login' => 'Se connecter',
        'register' => 'S\'inscrire',
        'logout' => 'Se déconnecter',
        'cart' => 'Panier',
        'language' => 'Langue',
        'theme' => 'Thème du site',
        'theme_default' => 'Comme sur le site',
        'theme_site' => 'par défaut',
        'home_aria' => 'Vers la page d\'accueil',
        'logo_alt' => 'Logo',
    ],

    'footer' => [
        'developer' => 'développeur',
        'rights' => 'tous droits réservés',
        'contacts' => 'Contacts',
        'write_us' => 'Écrivez-nous',
        'email' => 'Votre e-mail',
        'send' => 'Envoyer',
        'sending' => 'Ouverture…',
        'to_top' => 'Haut de page',
        'bad_email' => 'Saisissez une adresse e-mail valide',
        'mail_subject' => 'Message depuis le site',
        'mail_from' => 'Mon e-mail :',
        'mail_body' => 'Message :',
    ],

    'news' => [
        'all' => 'Toutes les actualités',
        'copy_link' => 'Copier le lien',
        'details' => 'Détails',
        'empty' => 'Aucune actualité publiée.',
        'in_stock' => 'En stock :',
        'latest' => 'Dernières publications',
        'link_copied' => 'Lien copié',
        'next' => 'Suivant',
        'no_category' => 'Sans catégorie',
        'no_image' => 'Pas d\'image',
        'prev' => 'Précédent',
        'preview' => 'Aperçu de l’article',
        'published_at' => 'Publié',
        'published_on' => 'Date de publication',
        'quantity' => 'Qté :',
        'read_more' => 'Lire la suite',
        'reading_time' => 'min de lecture',
        'section' => 'Actualités',
        'share' => 'Partager :',
    ],

    'search' => [
        'title' => 'Recherche',
        'title_with' => 'Recherche : :query',
        'subtitle' => 'Nous cherchons dans les actualités et les pages',
        'found' => 'Résultats trouvés :',
        'placeholder' => 'Que cherchez-vous ?',
        'submit' => 'Trouver',
        'start' => 'Lancer la recherche',
        'start_hint' => 'Saisissez un mot ou une phrase — nous afficherons les actualités et pages correspondantes.',
        'too_short' => 'Requête trop courte',
        'too_short_hint' => 'Saisissez au moins deux caractères.',
        'nothing' => 'Aucun résultat',
        'nothing_hint' => 'Aucune correspondance pour ":query".',
        'tip_spelling' => 'Vérifiez l\'orthographe',
        'tip_one_word' => 'Essayez un seul mot plutôt qu\'une phrase',
        'tip_general' => 'Utilisez un terme plus général',
        'pages_section' => 'Pages du site',
    ],

    'account' => [
        'edit_hint' => 'Contacts et informations légales — visibles uniquement par l\'administrateur',
        'g_contacts' => 'Contacts',
        'g_company' => 'Informations de l\'entreprise',
        'f_address' => 'Adresse',
        'f_phone' => 'Téléphone',
        'f_zip' => 'Code postal',
        'f_ceo' => 'Directeur général',
        'f_legal_addr' => 'Adresse légale',
        'f_actual_addr' => 'Adresse réelle',
        'f_okato' => 'OKATO',
        'save' => 'Enregistrer',
        'hello' => 'Bonjour, :name',
        'spent' => 'Montant total',
        'last_order' => 'Dernière commande',
        'history_hint' => 'Quand, depuis où et depuis quel appareil le compte a été utilisé',
        'history_empty_hint' => 'Les entrées apparaîtront après la première connexion.',
        'history_total' => 'Entrées au total',
        'h_success' => 'Réussie',
        'h_failed' => 'Échouée',
        'h_blocked' => 'Bloquée',
        'h_suspicious' => 'Suspecte',
        'h_unknown' => 'Inconnu',
        'pass_hint' => 'Choisissez un nouveau mot de passe pour vous connecter',
        'current_pass' => 'Mot de passe actuel',
        'new_pass' => 'Nouveau mot de passe',
        'confirm_pass' => 'Confirmation du mot de passe',
        'individual' => 'Particulier',
        'legal_entity_type' => 'Personne morale',
        'company' => 'Société',
        'amount' => 'Montant',
        'change_password' => 'Changer le mot de passe',
        'date' => 'Date',
        'delivery' => 'Livraison',
        'edit_profile' => 'Modifier le profil',
        'email' => 'E-mail',
        'history_empty' => 'Historique des connexions vide',
        'inn' => 'Numéro fiscal',
        'legal_entity' => 'Je suis une personne morale',
        'login_history' => 'Historique des connexions',
        'my_orders' => 'Mes commandes',
        'name' => 'Nom',
        'ogrn' => 'Numéro d\'enregistrement',
        'order_number' => 'N°',
        'orders_all' => 'Toutes les commandes',
        'orders_empty' => 'Vous n\'avez pas encore de commandes.',
        'orders_last' => 'Dernières commandes',
        'org_name' => 'Nom de la société',
        'organization' => 'Informations de la société',
        'payment' => 'Paiement',
        'quantity' => 'Qté',
        'status' => 'Statut',
        'title' => 'Espace personnel',
        'user_type' => 'Type de compte',
        'subtitle' => 'Vos données et votre historique de commandes',
        'profile' => 'Profil',
        'actions' => 'Actions',
        'edit' => 'Modifier les données',
        'edit_org' => 'Données de l\'entreprise',
        'change_pass' => 'Changer le mot de passe',
        'orders_count' => 'Commandes',
        'orders_none_hint' => 'Vos commandes apparaîtront ici après le premier achat.',
        'to_catalog' => 'Aller aux actualités',
        'st_pending' => 'En attente de paiement',
        'st_paid' => 'Payée',
        'st_completed' => 'Terminée',
        'st_cancelled' => 'Annulée',
        'st_unknown' => 'En traitement',
        'order_sum' => 'Total de la commande',
        'items' => 'articles',
    ],

    'common' => [
        'cms_tagline' => 'CMS modulaire',
        'fit_screen' => 'Ajuster à l\\x27écran',
        'download' => 'Télécharger',
        'home' => 'Accueil',
        'more' => 'Détails',
        'back' => 'Retour',
        'close' => 'Fermer',
        'open' => 'Ouvrir',
        'send' => 'Envoyer',
        'loading' => 'Chargement…',
    ],

    'cart' => [
        'free_delivery_from' => 'Livraison gratuite à partir de :sum ₽',
        'weight_exceeded' => 'La limite de poids est dépassée. Maximum : :limit kg',
        'region_unavailable' => 'La livraison dans cette région n\'est pas disponible',
        'no_delivery' => 'Aucun mode de livraison n\'a encore été configuré — contactez-nous.',
        'order_not_found' => 'La commande est introuvable.',
        'payment_confirmed' => 'Le paiement a été confirmé. Merci !',
        'payment_cancelled' => 'Le paiement a été annulé.',
        'payment_pending' => 'Nous attendons la confirmation du paiement. Le statut de la commande sera mis à jour dès la réponse de la banque.',
        'payment_start_failed' => 'Le paiement n\'a pas pu être lancé. La commande est créée — nous vous contacterons.',
        'no_payment' => 'Aucun moyen de paiement n\'a encore été configuré — contactez-nous.',
        'choose_delivery' => 'Choisissez un mode de livraison…',
        'choose_payment' => 'Choisissez un mode de paiement…',
        'delivery_method' => 'Mode de livraison',
        'empty' => 'Votre panier est vide.',
        'fee' => 'Frais :',
        'payment_method' => 'Mode de paiement',
        'quantity' => 'Qté :',
        'sum' => 'Sous-total :',
        'title' => 'Votre panier',
        'total' => 'Total à payer :',
    ],

    'a11y' => [
        'background' => 'Changer l\'arrière-plan',
        'colorblind' => 'Mode daltonien',
        'dyslexia' => 'Police adaptée à la dyslexie',
        'read_mode' => 'Mode lecture',
        'spacing' => 'Espacement élargi',
        'reset' => 'Tout réinitialiser',
        'widget' => 'Accessibilité',
        'title' => 'Réglages d\'accessibilité',
        'text_size' => 'Taille du texte',
        'speak_selection' => 'Lire à voix haute le texte sélectionné',
        'speak_page' => 'Lire à voix haute toute la page',
        'contrast' => 'Contraste élevé',
        'monochrome' => 'Monochrome',
        'sepia' => 'Sépia',
        'reading_mask' => 'Masque de lecture',
        'link_highlight' => 'Mettre les liens en évidence',
        'speak' => 'Lire à voix haute',
        'stop' => 'Arrêter',
        'on' => 'Activer',
        'off' => 'Désactiver',
        'show' => 'Afficher',
        'hide' => 'Masquer',
    ],
];
