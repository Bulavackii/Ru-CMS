<?php

return [

    'header' => [
        'theme_current' => 'in uso',
        'theme_fallback' => 'predefinito',
        'menu' => 'Menu',
        'tagline' => 'Contenuti & Gestione',
        'search' => 'Ricerca…',
        'search_action' => 'Cerca',
        'account' => 'Account',
        'account_title' => 'Area personale',
        'admin' => 'Amministrazione',
        'admin_title' => 'Pannello di amministrazione',
        'login' => 'Accedi',
        'register' => 'Registrati',
        'logout' => 'Esci',
        'cart' => 'Carrello',
        'language' => 'Lingua',
        'theme' => 'Tema del sito',
        'theme_default' => 'Come sul sito',
        'theme_site' => 'predefinito',
        'home_aria' => 'Alla pagina iniziale',
        'logo_alt' => 'Logo',
    ],

    'footer' => [
        'developer' => 'sviluppatore',
        'rights' => 'tutti i diritti riservati',
        'contacts' => 'Contatti',
        'write_us' => 'Scrivici',
        'email' => 'La tua e-mail',
        'send' => 'Invia',
        'sending' => 'Apertura…',
        'to_top' => 'Torna su',
        'bad_email' => 'Inserisci un indirizzo e-mail valido',
        'mail_subject' => 'Messaggio dal sito',
        'mail_from' => 'La mia e-mail:',
        'mail_body' => 'Messaggio:',
    ],

    'news' => [
        'all' => 'Tutte le notizie',
        'copy_link' => 'Copia link',
        'details' => 'Dettagli',
        'empty' => 'Nessuna notizia pubblicata.',
        'in_stock' => 'Disponibili:',
        'latest' => 'Ultime pubblicazioni',
        'link_copied' => 'Link copiato',
        'next' => 'Successivo',
        'no_category' => 'Senza categoria',
        'no_image' => 'Nessuna immagine',
        'prev' => 'Precedente',
        'preview' => 'Anteprima della notizia',
        'published_at' => 'Pubblicato',
        'published_on' => 'Data di pubblicazione',
        'quantity' => 'Qtà:',
        'read_more' => 'Leggi tutto',
        'reading_time' => 'min di lettura',
        'section' => 'Notizie',
        'share' => 'Condividi:',
    ],

    'search' => [
        'title' => 'Ricerca',
        'title_with' => 'Ricerca: :query',
        'subtitle' => 'Cerchiamo tra notizie e pagine del sito',
        'found' => 'Risultati trovati:',
        'placeholder' => 'Cosa cerchi?',
        'submit' => 'Trova',
        'start' => 'Inizia la ricerca',
        'start_hint' => 'Inserisci una parola o una frase — mostreremo notizie e pagine corrispondenti.',
        'too_short' => 'Richiesta troppo breve',
        'too_short_hint' => 'Inserisci almeno due caratteri.',
        'nothing' => 'Nessun risultato',
        'nothing_hint' => 'Nessuna corrispondenza per ":query".',
        'tip_spelling' => 'Controlla l\'ortografia',
        'tip_one_word' => 'Prova una sola parola invece di una frase',
        'tip_general' => 'Usa un termine più generico',
        'pages_section' => 'Pagine del sito',
    ],

    'account' => [
        'edit_hint' => 'Contatti e dati aziendali — visibili solo all\'amministratore',
        'g_contacts' => 'Contatti',
        'g_company' => 'Dati aziendali',
        'f_address' => 'Indirizzo',
        'f_phone' => 'Telefono',
        'f_zip' => 'CAP',
        'f_ceo' => 'Amministratore delegato',
        'f_legal_addr' => 'Indirizzo legale',
        'f_actual_addr' => 'Indirizzo effettivo',
        'f_okato' => 'OKATO',
        'save' => 'Salva',
        'hello' => 'Ciao, :name',
        'spent' => 'Totale speso',
        'last_order' => 'Ultimo ordine',
        'history_hint' => 'Quando, da dove e da quale dispositivo si è acceduto all\'account',
        'history_empty_hint' => 'Le voci appariranno dopo il primo accesso.',
        'history_total' => 'Voci in totale',
        'h_success' => 'Riuscito',
        'h_failed' => 'Fallito',
        'h_blocked' => 'Bloccato',
        'h_suspicious' => 'Sospetto',
        'h_unknown' => 'Sconosciuto',
        'pass_hint' => 'Scegli una nuova password per accedere',
        'current_pass' => 'Password attuale',
        'new_pass' => 'Nuova password',
        'confirm_pass' => 'Conferma della password',
        'individual' => 'Privato',
        'legal_entity_type' => 'Persona giuridica',
        'company' => 'Azienda',
        'amount' => 'Totale',
        'change_password' => 'Cambia password',
        'date' => 'Data',
        'delivery' => 'Consegna',
        'edit_profile' => 'Modifica profilo',
        'email' => 'E-mail',
        'history_empty' => 'Cronologia accessi vuota',
        'inn' => 'Codice fiscale',
        'legal_entity' => 'Sono una persona giuridica',
        'login_history' => 'Cronologia accessi',
        'my_orders' => 'I miei ordini',
        'name' => 'Nome',
        'ogrn' => 'Numero di registrazione',
        'order_number' => 'N.',
        'orders_all' => 'Tutti gli ordini',
        'orders_empty' => 'Non hai ancora ordini.',
        'orders_last' => 'Ordini recenti',
        'org_name' => 'Ragione sociale',
        'organization' => 'Dati aziendali',
        'payment' => 'Pagamento',
        'quantity' => 'Qtà',
        'status' => 'Stato',
        'title' => 'Area personale',
        'user_type' => 'Tipo di account',
        'subtitle' => 'I tuoi dati e lo storico degli ordini',
        'profile' => 'Profilo',
        'actions' => 'Azioni',
        'edit' => 'Modifica i dati',
        'edit_org' => 'Dati dell\'azienda',
        'change_pass' => 'Cambia la password',
        'orders_count' => 'Ordini',
        'orders_none_hint' => 'I tuoi ordini appariranno qui dopo il primo acquisto.',
        'to_catalog' => 'Vai alle notizie',
        'st_pending' => 'In attesa di pagamento',
        'st_paid' => 'Pagato',
        'st_completed' => 'Completato',
        'st_cancelled' => 'Annullato',
        'st_unknown' => 'In elaborazione',
        'order_sum' => 'Totale ordine',
        'items' => 'articoli',
    ],

    'common' => [
        'cms_tagline' => 'CMS modulare',
        'fit_screen' => 'Adatta allo schermo',
        'download' => 'Scarica',
        'home' => 'Home',
        'more' => 'Dettagli',
        'back' => 'Indietro',
        'close' => 'Chiudi',
        'open' => 'Apri',
        'send' => 'Invia',
        'loading' => 'Caricamento…',
    ],

    'cart' => [
        'free_delivery_from' => 'Spedizione gratuita da :sum ₽',
        'weight_exceeded' => 'Il limite di peso è superato. Massimo: :limit kg',
        'region_unavailable' => 'La spedizione in questa regione non è disponibile',
        'no_delivery' => 'Nessun metodo di spedizione è ancora configurato — contattaci.',
        'order_not_found' => 'Ordine non trovato.',
        'payment_confirmed' => 'Il pagamento è stato confermato. Grazie!',
        'payment_cancelled' => 'Il pagamento è stato annullato.',
        'payment_pending' => 'Attendiamo la conferma del pagamento. Lo stato dell\'ordine verrà aggiornato non appena la banca risponderà.',
        'payment_start_failed' => 'Non è stato possibile avviare il pagamento. L\'ordine è stato creato — ti contatteremo.',
        'no_payment' => 'Nessun metodo di pagamento è ancora configurato — contattaci.',
        'choose_delivery' => 'Scegli un metodo di consegna…',
        'choose_payment' => 'Scegli un metodo di pagamento…',
        'delivery_method' => 'Metodo di consegna',
        'empty' => 'Il tuo carrello è vuoto.',
        'fee' => 'Commissione:',
        'payment_method' => 'Metodo di pagamento',
        'quantity' => 'Qtà:',
        'sum' => 'Totale parziale:',
        'title' => 'Il tuo carrello',
        'total' => 'Totale da pagare:',
    ],

    'a11y' => [
        'background' => 'Cambia lo sfondo',
        'colorblind' => 'Modalità per daltonici',
        'dyslexia' => 'Carattere per dislessia',
        'read_mode' => 'Modalità lettura',
        'spacing' => 'Spaziatura maggiore',
        'reset' => 'Reimposta tutto',
        'widget' => 'Accessibilità',
        'title' => 'Impostazioni di accessibilità',
        'text_size' => 'Dimensione del testo',
        'speak_selection' => 'Leggi ad alta voce il testo selezionato',
        'speak_page' => 'Leggi ad alta voce l\'intera pagina',
        'contrast' => 'Contrasto elevato',
        'monochrome' => 'Monocromatico',
        'sepia' => 'Seppia',
        'reading_mask' => 'Maschera di lettura',
        'link_highlight' => 'Evidenzia i collegamenti',
        'speak' => 'Leggi ad alta voce',
        'stop' => 'Ferma',
        'on' => 'Attiva',
        'off' => 'Disattiva',
        'show' => 'Mostra',
        'hide' => 'Nascondi',
    ],
];
