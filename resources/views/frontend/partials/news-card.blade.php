...
<div class="fx-card flex flex-col w-full h-full overflow-hidden">
    <div class="w-full h-40 sm:h-44 md:h-48 lg:h-52 xl:h-56 bg-gray-100 dark:bg-gray-700 overflow-hidden flex items-center justify-center">
        @if ($isVideo)
            <video
                controls
                preload="metadata"
    ...
    <div class="flex flex-col flex-grow p-4 sm:p-5 overflow-hidden">
        ...
        <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-2 flex items-center gap-1.5">
            <i class="far fa-calendar-alt fx-ico"></i> {{ optional($news->created_at)->format('d.m.Y') }}
        </p>
        ...
        <a href="{{ route('news.show', $news->slug) }}" class="fx-btn mt-auto w-full py-2 sm:py-2.5 text-xs sm:text-sm font-medium">
