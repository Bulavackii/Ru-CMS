<div class="my-12 max-w-screen-2xl mx-auto px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 2xl:px-16">
    ...
    <div class="fx-section-head">
        <span class="fx-badge"><i class="fab fa-css3-alt"></i></span>
        <div>
            <h2 class="fx-section-title">{{ $title ?? __('frontend.templates.css') }}</h2>
        </div>
    </div>
    ...
    @if ($newsList->count())
        ...
        <div class="flex flex-wrap justify-center gap-8">
            @foreach ($newsList as $news)
                ...
                <div class="relative flex flex-col p-5 border border-gray-100 hover:border-gray-300 shadow-md hover:shadow-lg transition-all bg-white rounded-2xl max-w-xs w-full">
                    {{-- 🎨 бейдж "Урок" --}}
                    <div class="absolute -top-3 right-3 z-10 bg-white border-2 border-indigo-600 text-indigo-600 text-xs font-bold px-3 py-1 rounded-full shadow-md select-none">
                        🎨 CSS
                    </div>
                        ...
                        <div class="absolute top-3 left-3 z-10 flex flex-wrap gap-1">
                            @foreach ($news->categories as $category)
                                <a href="{{ url('/?category_' . $news->template . '=' . $category->id) }}"
                                   class="bg-indigo-100 text-indigo-800 text-xs font-semibold px-2 py-1 rounded-full hover:underline select-none">
                                    {{ $category->title }}
                                </a>
                            @endforeach
                        </div>
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 mb-1 leading-tight break-words line-clamp-2">
                        <a href="{{ route('news.show', $news->slug) }}" class="hover:text-indigo-600 transition">
                            {{ $news->title }}
                        </a>
                    </h3>
                    ...
                    <a href="{{ route('news.show', $news->slug) }}"
                       class="mt-auto block text-center text-sm bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2.5 rounded-lg transition shadow">
                        Читать далее →
                    </a>
                </div>
            @endforeach
                ...
                <nav class="flex items-center space-x-2 rtl:space-x-reverse" role="navigation">
                    ...
                    @foreach ($newsList->getUrlRange(1, $newsList->lastPage()) as $page => $url)
                        @if ($page == $newsList->currentPage())
                            <span class="px-3 py-1.5 bg-indigo-600 text-white rounded-md text-sm font-semibold shadow">
                                {{ $page }}
                            </span>
                        @else
                            <a href="{{ $url }}"
        ...
        @endif
    @else
        <p class="text-center text-gray-500">Нет уроков по CSS.</p>
    @endif
</div>
