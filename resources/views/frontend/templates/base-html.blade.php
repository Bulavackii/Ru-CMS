<div class="my-12 max-w-screen-xl mx-auto px-4">
    ...
    @if ($newsList->count())
        <div class="flex flex-wrap justify-center gap-8">
            @foreach ($newsList as $news)
                ...
                <div class="relative flex flex-col p-5 border border-gray-100 hover:border-gray-300 shadow-md hover:shadow-lg transition-all bg-white rounded-2xl max-w-xs w-full">
                    <div class="absolute -top-3 right-3 z-10 bg-white border-2 border-red-600 text-red-600 text-xs font-bold px-3 py-1 rounded-full shadow-md select-none">
                        📕 HTML
                    </div>
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 mb-1 leading-tight break-words line-clamp-2">
                        <a href="{{ route('news.show', $news->slug) }}" class="hover:text-red-600 transition">
                            {{ $news->title }}
                        </a>
                    </h3>
        ...
        @endif
    @else
        <p class="text-center text-gray-500">Нет уроков по HTML.</p>
    @endif
</div>
