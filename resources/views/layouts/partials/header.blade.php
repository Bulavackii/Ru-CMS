  ...
  <style>
    ...
    @media (max-width: 1024px), (max-height: 500px){
      ...
      /* ── Телефон в альбомной ориентации ───────────────────────────────
         Экран широкий, но НИЗКИЙ: 896×414. Колонкой пять пунктов вместе с
         поиском занимали 328 из 414 — раскрытое меню закрывало страницу
         целиком, и до содержимого приходилось его закрывать.

         В одну строку пункты не поставить: у них бывают подменю, которые
         на этом пороге раскрыты сразу. Поэтому сетка: пункт со своей
         веткой — это ячейка, и пять пунктов ложатся в два ряда вместо
         пяти. Порог 640 отсекает телефон в портрете (там ширины на
         колонки нет) и оставляет правило только альбомной ориентации. */
      @media (max-height: 500px) and (min-width: 640px){
        ...
        /* Ячейки соседних колонок разделяет столбец, а не линия под
           пунктом: снизу у ячейки может стоять целая ветка. */
        .hdr-row2 .header-nav > .menu-item--root{
          border-right:1px solid var(--surface-bd,#eef0f5);
          padding-right:.75rem;
        }
        .hdr-row2 .header-nav > .menu-item--root:nth-child(3n),
        .hdr-row2 .header-nav > .menu-item--root:last-child{
          border-right:0; padding-right:0;
        }
        /* Строка ниже: в два ряда влезает вдвое больше, а высоту экрана
           надо беречь. 44 — нижняя граница зоны нажатия, ниже нельзя. */
        .hdr-row2 .header-nav .menu-link--root{
          min-height:44px; padding:.45rem .5rem; border-bottom:0;
        }
        .hdr-row2 .header-nav .menu-link--l2,
