...
<header class="fx-header relative text-sm z-10" x-data="{ navOpen: false }"
  ...
  <style>
    ...
    @media (max-width: 1024px), (max-height: 500px){
      ...
      /* ⚠️ Полю поиска нужно РОВНО 16 пикселей, и это не общий порог в 12.
         Safari на iPhone сам приближает страницу при фокусе в поле мельче
         16 и обратно её не отпускает — то есть после одного касания поиска
         сайт остаётся в увеличенном виде. Замер показывал 13.6.
         `user-scalable=no` не выход: ломает доступность, и Safari его
         всё равно игнорирует. */
      .hdr-search input,
      input[name="q"]{ font-size:16px }
