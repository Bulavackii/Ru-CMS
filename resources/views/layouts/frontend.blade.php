...
@endif
    ...
    <style>
        ...
        @media (max-width: 1023px), (max-height: 500px){
            /* ⚠️ Запас под плавающую кнопку даём ПОДВАЛУ, а не странице.
               Отступ на body лежал ниже подвала, то есть вне его фона, и
               под ним появлялась пустая полоса в 80 пикселей — выглядело
               как обрыв вёрстки. Тот же запас внутри подвала продолжает
               его фон, и низ страницы остаётся сплошным.
               Красим саму `.f-body`: именно она рисует заливку подвала. */
            body .f-body{ padding-bottom:4.5rem; }
            ...
            .pg-chip, .nw-chip, .pr-chip, .mag-chip,
            .f-brand-chip, .f-social-label, .pr-card__stock{ font-size:12px; }
...
<body class="fx-sharp relative text-gray-800 dark:text-gray-100 min-h-screen flex flex-col overflow-x-hidden bg-white dark:bg-gray-900 transition-colors duration-200 {{ $pageTheme ? 'fx-themed' : '' }} {{ $themeIsDark ? 'fx-theme-dark' : '' }}"
    ...
    <script>
        (function () {
            ...
            if (!window.matchMedia('(max-width: 1023px), (max-height: 500px)').matches) return;
