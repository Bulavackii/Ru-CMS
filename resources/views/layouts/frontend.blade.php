...
@endif
    ...
    <style>
        ...
        /* ⚠️ Класс остался ради виджета спецвозможностей: режим чтения
           прячет `.fx-topbar` наравне с меню и подвалом. Сам элемент из
           макета убран — см. пояснение в теле страницы. */
        ...
        @media (max-width: 1024px), (max-height: 500px){
            /* ── Пол кегля: 12 пикселей ───────────────────────────────────
               Мельче браузер на телефоне предлагает увеличить страницу
               целиком, ломая вёрстку. Правило то же, что в панели, но
               оно там и осталось — на сайте пола не было вовсе.
               Перечислены ровно те подписи, которые нашёл замер по семи
               размерам: рубрика журнальной карточки (10.56), строка под
               названием раздела (10.88) и метка «Свежее» у выпуска
               (10.56). Все три стоят на витрине главной, то есть это
               первое, что видит покупатель с телефона.
               Селектор с `body.fx-sharp` (0-2-1) намеренно: стили
               `.mag-rubric` и `.rl-fresh` объявлены в шаблонах, то есть
               НИЖЕ по документу, и при равной силе перебили бы макет. */
            body.fx-sharp .mag-rubric,
            body.fx-sharp .rl-fresh,
            body.fx-sharp .fx-section-head .fx-section-sub{ font-size:.75rem; }
        }
...
<body class="fx-sharp relative text-gray-800 dark:text-gray-100 min-h-screen flex flex-col overflow-x-hidden bg-white dark:bg-gray-900 transition-colors duration-200 {{ $pageTheme ? 'fx-themed' : '' }} {{ $themeIsDark ? 'fx-theme-dark' : '' }}"
    ...
    <div id="wrapper" class="relative z-10 flex flex-col min-h-screen">
        {{-- 🔴 Верхней акцент-полосы здесь больше НЕТ.
             Она шла у самого края окна над шапкой и читалась не как часть
             сайта, а как полоса загрузки браузера — владелец указал на неё
             как на лишнюю во ВСЕХ темах. Правило `.fx-topbar` оставлено:
             тем же цветом рисуется `.fx-accent-bar` внутри страницы, и
             переменная `--fx-bar` у них общая. --}}
    ...
    <script>
        (function() {
            ...
            // 🔴 Мерить РАНЬШЕ, чем загрузился шрифт значков, — значит объявить
            // сломанным весь набор.
            //
            // Пока шрифт грузится, браузер держит текст невидимым (FOIT): у
            // элемента со значком ширина ровно 0. Проверка это видела и молча
            // подменяла значок заглушкой — НАВСЕГДА, потому что подменённый
            // элемент помечается data-fallback и второй раз не проверяется.
            // Владелец увидел это на теме «Алый»: половина значков в шапке и
            // подвале превратилась в глухие кружки и квадраты.
            //
            // Событие `load` тут не годится: оно ждёт стили и картинки, но не
            // веб-шрифты. Ждать нужно именно `document.fonts.ready`.
            const измерить = () => setTimeout(fix, 60);

            if (document.fonts && document.fonts.ready) {
                document.fonts.ready.then(измерить);
            } else {
                window.addEventListener('load', измерить);
            }
        })();
    </script>
