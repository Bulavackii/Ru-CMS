...
/* ── Требует внимания ───────────────────────────────────────────── */
    ...
    /* ⚠️ На сенсорных аура выключается совсем.
       Это два размытых пятна (blur 70px) с бесконечной анимацией
       `transform` и `will-change` — то есть постоянно живой
       композиторский слой поверх шапки. Именно такие слои у владельца
       давали рябь (та же история была с полноэкранным редактором), плюс
       на телефоне это непрерывная работа видеоядра ради украшения,
       которое на маленьком экране почти не видно.

       Проверка по всем 57 страницам панели показала, что после снятия
       размытия и тяжёлых переходов это единственное оставшееся место с
       постоянным слоем. */
    @media (max-width: 1024px), (max-height: 500px){
        .dash-aurora{ display:none }
    }
