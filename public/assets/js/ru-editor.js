...
(function (window, document) {
    ...
    Editor.prototype = {
        ...
        undo: function () {
            // Снимок состояния делается с задержкой: подряд набранные буквы
            // не должны плодить по шагу истории на каждую. Но то, что набрано
            // ПОСЛЕ последнего снимка, в историю ещё не попало — и первое
            // нажатие выбрасывало его вместе с предыдущим шагом, отматывая
            // будто на два. Поэтому сначала запоминаем текущее состояние:
            // тогда первая отмена возвращает ровно к тому, что было до
            // последнего набора. Повторный снимок без изменений ничего не
            // добавляет — внутри есть проверка.
            this._snapshot();
