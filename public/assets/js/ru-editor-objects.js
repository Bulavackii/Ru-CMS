...
(function (window, document) {
    ...
    function makeInline(box) {
        if (box.classList.contains('pc-audio')) {
            ...
            if (!box.style.width) {
                // Ширина по умолчанию — не во всю строку, и это главное для
                // выравнивания: полосу шириной с колонку двигать некуда, и
                // кнопки не делали ничего видимого. Пятьсот шестьдесят
                // пикселей — обычный размер такого проигрывателя; на узком
                // экране берётся вся доступная ширина.
                //
                // Ставим только когда своей ширины нет: растянутую автором за
                // ручки полосу трогать нельзя.
                //
                // border-box обязателен: у полосы есть поля и рамка, и без
                // него ширина вылезала бы за край текста на их толщину.
