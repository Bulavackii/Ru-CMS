...
(function (window) {
    ...
    function buildPlayer(tag, url, values) {
        ...
        // Обёртка с классом: по ней сайт узнаёт проигрыватель и оформляет
        // его, а звук вдобавок заменяет на свой — родная панель выглядит в
        // каждом браузере по-своему. Без обёртки пришлось бы опознавать
        // проигрыватели по голому тегу, а он встречается и в чужой вставке.
        var cls = tag === 'audio' ? 'pc-player pc-audio' : 'pc-player pc-player--video';

        return '<div class="' + cls + '"><' + tag + ' ' + attrs.join(' ') + '></' + tag + '></div><p><br></p>';
    }
    ...
    /**
     * Показ проигрывателей в рамке редактора.
     *
     * Панель звука рисует тот же скрипт, что и на сайте
     * (content-players.js), поэтому в редакторе видно ровно то, что увидит
     * посетитель, — без второй копии оформления, которая неизбежно разошлась
     * бы с первой.
     *
     * Сборка идёт по изменению содержимого: вставка могла появиться и после
     * загрузки — из диалога, из буфера, отменой правки. Повторный вызов
     * безвреден, скрипт помечает уже собранные проигрыватели.
     */
    RuEditor.registerPlugin('player-preview', {
        init: function (editor) {
            var upgrade = function () {
                if (window.ruUpgradePlayers) {
                    window.ruUpgradePlayers(editor.doc);
                }
            };

            editor.on('change', upgrade);
            upgrade();
        }
    });
