        ...
        <dl class="ord-list">
            ...
            <div><dt>{{ __('admin.orders.f_delivery') }}</dt>
                <dd>
                    ...
                    @else
                        {{-- Прочерк здесь читался как «забыли выбрать». У заказа
                             из одних услуг доставки нет по замыслу — так и
                             пишем, иначе владелец идёт выяснять несуществующую
                             ошибку. --}}
                        <span class="ord-none">{{ __('admin.orders.delivery_not_needed') }}</span>
                    @endif
                </dd></div>
