...
class PaymentGatewayService
{
    ...
    public function createPayment(Order $order, PaymentMethod $paymentMethod, array $options = []): array
    {
        ...
        // 🔴 Драйвер-заглушка до покупателя не доходит НИКОГДА.
        //
        // Проверка стоит здесь, а не только в форме панели: способ может
        // оказаться включённым мимо формы — сидером, выгрузкой, правкой в
        // базе руками. Отказ — безопасная сторона: заказ останется
        // неоплаченным и будет виден владельцу, тогда как нерабочая
        // «оплата» выглядит для покупателя как успешная.
        if ($paymentMethod->isStub()) {
            throw new \Exception(
                "Способ оплаты '{$paymentMethod->code}' не готов к приёму денег: драйвер написан заглушкой."
            );
        }
    ...
    public function handleWebhook(string $gatewayCode, array $data): bool
    {
        ...
        // Заслон дублируется здесь намеренно. Маршрут уведомлений публичный
        // и без CSRF, поэтому «подтвердить оплату» по телу запроса не должен
        // уметь ни один недописанный драйвер — даже если однажды его
        // handleWebhook() перепишут, не подумав о проверке у банка.
        if ($paymentMethod->isStub()) {
            Log::warning('Уведомление отброшено: драйвер не готов подтверждать оплату', [
                'gateway' => $gatewayCode,
            ]);
