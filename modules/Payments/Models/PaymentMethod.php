...
class PaymentMethod extends Model
{
    ...
    /**
     * 🔴 Готовность драйверов к бою.
     *
     * Перечислены только те, у кого есть оговорка. Всё остальное считается
     * готовым: у офлайновых способов (наличные, счёт) драйвера нет вовсе —
     * ломаться там нечему.
     *
     * `stub` — драйвер написан ЗАГЛУШКОЙ. У СБП «QR-код» — это base64 от
     *   строки, а не картинка; статус платежа читается из НАШЕЙ базы, а не у
     *   банка; уведомления намеренно не обрабатываются (иначе публичный
     *   маршрут без CSRF позволял бы объявить заказ оплаченным). Включить
     *   такой способ = показать покупателю нерабочую оплату.
     *
     * `untested` — код полный и обращается к настоящему API банка, но живой
     *   приём денег ни разу не проверялся: для этого нужен мерчант-договор,
     *   а все тесты идут на `Http::fake()`. Включать можно, но сначала
     *   провести пробный платёж в тестовом режиме.
     *
     * ⚠️ Ключ — КОД способа, а если его нет, то тип. Старые коды
     * (`tinkoff`, `sberbank`) оставлены рядом с новыми: у кого-то способ
     * заведён ещё под прежним именем.
     */
    public const READINESS = [
        ...
        'sberpay'       => 'untested',
        'sberbank'      => 'untested',
        'cloudpayments' => 'untested',
        'robokassa'     => 'untested',
        'qiwi'          => 'untested',
    ];

    /** Готовность этого способа: `ready` | `untested` | `stub`. */
    public function readiness(): string
    {
        $код = $this->code ?: $this->type;

        return self::READINESS[$код] ?? self::READINESS[$this->type] ?? 'ready';
    }

    /** Способ нельзя включать: его драйвер — заглушка. */
    public function isStub(): bool
    {
        return $this->readiness() === 'stub';
    }
