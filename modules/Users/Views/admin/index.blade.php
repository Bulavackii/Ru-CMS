...
@section('content')
    ...
    {{-- ── Массовые операции.
         ...
         может жить отдельно. ── --}}
    <form method="POST" action="{{ route('admin.users.bulkAction') }}" id="bulkForm" class="admin-card p-4 mb-5">
        @csrf
        <div class="flex flex-wrap items-center gap-3">
            ...
            <select name="role_id" id="bulkRole" style="display:none;"
                    class="border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white px-3 py-2 text-sm
                           focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
            ...
            <span id="bulkCounter" class="text-xs text-gray-500 dark:text-gray-400 ml-auto">Ничего не выбрано</span>
        </div>
    </form>
...
<script>
    (function () {
        ...
        function refresh() {
            ...
            if (counter) counter.textContent = n ? `Выбрано: ${n} ${word(n)}` : 'Ничего не выбрано';
            if (submitBtn) submitBtn.disabled = !(n > 0 && actionSel && actionSel.value);
            if (selectAll) {
        ...
        actionSel?.addEventListener('change', function () {
            ...
            if (roleSel) {
                roleSel.style.display = needRole ? 'block' : 'none';
