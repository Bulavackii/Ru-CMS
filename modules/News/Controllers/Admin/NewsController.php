...
class NewsController extends Controller
{
     ...
     * демо-страница «Nexum Core». Редактор выбирал из шестнадцати пунктов,
    ...
    /**
     * Что ВЕЗУТ, а что ОКАЗЫВАЮТ.
     *
     * 🔴 Разница не косметическая. До этого списка корзина предлагала доставку
     ...
     *
     * В списке только то, что физически пересылается. Всё остальное с ценой
     * (услуги, приёмы, работы) покупается без доставки вовсе: корзина не
     * показывает шаг доставки, не спрашивает адрес и город и не берёт за неё
     * денег.
     *
     * ⚠️ Список один на весь проект — его читают и форма материала, и корзина,
     * и оформление заказа. Вторая копия рано или поздно разошлась бы, а цена
     * расхождения здесь — деньги покупателя.
     */
    public const SHIPPABLE_TEMPLATES = ['products'];

    /**
     * Вес носят только физические товары — ровно те же, что везут.
     *
     ...
     */
    public const WEIGHT_TEMPLATES = self::SHIPPABLE_TEMPLATES;
