...
<p>Тоже в разделе SEO. По умолчанию сайт открыт для поисковиков. Пока сайт не готов, закройте его от индексации, чтобы черновики не попали в выдачу.</p>',
        ...
        $поШаблонам = [
            ['template' => 'ourworks', 'slug' => 'usluga-montazh', 'title' => 'Монтаж «под ключ»',
             'content' => '<p>Берём объект целиком: замер, доставка, установка и сдача. Работаем по договору, сроки фиксируем письменно.</p><ul><li>Выезд замерщика в день обращения</li><li>Материалы и техника — наши</li><li>Гарантия на работы 24 месяца</li><li>Акт и закрывающие документы сразу</li></ul>'],

            ['template' => 'faq', 'slug' => 'vopros-dostavka', 'title' => 'Доставка и оплата',
             'content' => '<h3>Сколько идёт доставка?</h3><p>По городу — от нуля до двух дней, по стране — от трёх до четырнадцати в зависимости от службы.</p><h3>Когда платить?</h3><p>При получении или переводом по счёту. Онлайн-оплата подключается в панели, если нужны карты.</p><h3>Можно ли вернуть?</h3><p>Да, в течение четырнадцати дней, если товар не был в использовании.</p>'],

            // Оценка у отзыва — тот же rating, что у обзоров «Игр», шкала 0..10.
            ['template' => 'reviews', 'slug' => 'otzyv-klienta', 'rating' => 9.0, 'title' => 'Отзыв: работали второй раз',
             'content' => '<p>Обращались повторно и снова остались довольны. Приехали в назначенное время, всё сделали за один день и убрали за собой.</p><p>Отдельно отмечу, что <strong>смету не меняли по ходу работ</strong> — назвали цену на замере, её и заплатили. В прошлый раз было так же.</p><p>Рекомендую тем, кому важны сроки.</p>'],
