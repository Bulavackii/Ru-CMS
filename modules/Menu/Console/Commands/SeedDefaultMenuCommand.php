...
class SeedDefaultMenuCommand extends Command
{
    ...
    public static function definitions(): array
    {
        return [
            ...
            /*
             * Контакты. Как и соцсети, раньше были вписаны прямо в шаблон
             * подвала: почта, телефон и адрес правились только в коде.
             *
             * Мелкая подпись над значением («Написать», «Позвонить», «Адрес»)
             * НЕ хранится отдельным полем: она выводится из схемы ссылки —
             * mailto:, tel: или всё остальное. Так у пункта остаётся обычная
             * пара «название и адрес», без выдуманных полей, которых больше
             * нигде в меню нет.
             */
            [
                'position' => 'contacts',
                'title'    => 'Контакты',
                'items'    => [
                    ['title' => 'info@example.com',              'url' => 'mailto:info@example.com', 'icon' => 'mail'],
                    ['title' => '+7 (900) 123-45-67',            'url' => 'tel:+79001234567',        'icon' => 'phone'],
                    ['title' => 'г. Москва, Тверская улица, 7',  'url' => 'https://yandex.ru/maps/?text=' . rawurlencode('Москва, Тверская улица, 7'), 'icon' => 'map'],
                ],
            ],

            /*
             * Соцсети. Раньше их адреса лежали в config/app.php и правились
             * только в файле — добавить новую сеть без разработчика было
             * нельзя. Теперь это обычное меню: пункт добавляется, правится и
             * удаляется в разделе «Меню», как всё остальное.
             *
             * Значки берутся по слагу ссылки: у четырёх знакомых сетей в
             * подвале свои фирменные глифы, у любой добавленной — значок из
             * поля пункта (см. social_links() в app/helpers.php).
             *
             * Адреса те же, что были в конфиге: после обновления подвал
             * выглядит ровно так же, как до него.
             */
            [
                'position' => 'social',
                'title'    => 'Соцсети',
                'items'    => [
                    ['title' => 'ВКонтакте', 'url' => config('app.social.vk'),     'icon' => 'vk'],
                    ['title' => 'MAX',       'url' => config('app.social.max'),    'icon' => 'max'],
                    ['title' => 'Rutube',    'url' => config('app.social.rutube'), 'icon' => 'rutube'],
                    ['title' => 'GitHub',    'url' => config('app.social.github'), 'icon' => 'github'],
                ],
            ],
