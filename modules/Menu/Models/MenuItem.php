...
class MenuItem extends Model
{
    ...
    /**
     * Пункты, которые есть смысл показывать посетителю.
     *
     * Пункт типа «страница» хранит её идентификатор в linked_id. Страницу
     * удалили или сняли с публикации — связи больше нет, и frontendUrl()
     * отдаёт '#': в меню оставался ЖИВОЙ НА ВИД пункт со старым названием,
     * который никуда не ведёт. Владелец так и сообщил: «удалил страницу в
     * админке, а она на фронте до сих пор висит».
     *
     * ⚠️ Условие пишется через orWhereHas, а не проверкой в шаблоне:
     * фильтровать нужно ДО кеша, иначе в кеше так и лежит пункт-призрак.
     *
     * Сам пункт при этом НЕ удаляется: его завёл владелец, и решать, убрать
     * его или перенаправить на другую страницу, тоже ему. Скрываем только
     * показ на сайте, в панели пункт виден как был.
     */
    public function scopeVisible($query)
    {
        return $query->where(function ($q) {
            $q->where('type', '!=', 'page')
              ->orWhereHas('linkedPage', fn ($p) => $p->where('published', true));
        });
    }
