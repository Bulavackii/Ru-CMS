...
class FreshInstallCardMetaTest extends TestCase
{
    ...
    public function test_every_card_block_shows_a_date(): void
    {
        ...
        // ⚠️ «Клиники» здесь больше нет, и это ОСОЗНАННО, а не подгонка под
        // результат: дата публикации у медицинской услуги бессмысленна —
        // пациенту неважно, когда завели описание приёма, — и она стояла на
        // самом видном месте карточки. Убрана вместе с классом.
        //
        // Смысл проверки при этом сохраняется: она ловит пустую дату, то есть
        // колонку created_at, не попавшую в select() запроса. Для этого хватает
        // блоков, которые дату показывают.
        $blocks = [
