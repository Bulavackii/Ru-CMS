...
class CaptchaPresetTest extends TestCase
{
    ...
    public function test_answer_never_appears_in_the_markup(string $type, array $options = []): void
    {
        ...
        // ⚠️ Ищем ответ только в том, что бот может ПРОЧИТАТЬ как ответ.
        // Из разметки убираются две вещи, где короткое число оказывается
        // случайно, и тест из-за них плавал — падал примерно раз на прогон
        // и проходил при повторе:
        //   • сам идентификатор экземпляра (он случайный, цифры внутри);
        //   • полезная нагрузка base64 у картинки — это пиксели, а не текст,
        //     прочитать в ней «12» нельзя.
        $видимое = preg_replace('~name="captcha_id" value="[^"]*"~', '', $html);
        $видимое = preg_replace('~data:image/[a-z+]+;base64,[A-Za-z0-9+/=]+~', '', $видимое);
